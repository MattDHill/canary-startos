...
This repo packages [Canary](https://github.com/schjonhaug/canary) for StartOS.
...
Canary ships as two coordinated upstream images pinned by `dockerTag` in `startos/manifest/index.ts`. Both share the same Canary version and must be bumped together.
